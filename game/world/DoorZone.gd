extends Area2D
## Single doorway name plate (one per building, at the door).
...
var _facing: String = "south" # south | west | north
var _t: float = 0.0
...
func _layout_chrome() -> void:
	# Keep one plate glued to the doorway for each facing.
	if plate == null or label == null or arrow == null:
		return
	match _facing:
		"west":
			# Plate just left of west-facing door (on the path).
		...
		"north":
			# Dock: plate on the path north of the door.
		...
		_:
			# Default south-facing doors: plate above threshold.
			plate.offset_left = -78.0
...
func refresh() -> void:
	if location_id == "__exit__":
		...
		if arrow:
			...
			arrow.visible = true
		if plate:
			plate.color = Color(0.95, 0.90, 0.78, 0.95)
		return
	...
	if arrow:
		arrow.visible = false
	if plate:
		plate.color = Color(0.78, 0.74, 0.66, 0.94) if locked else Color(0.95, 0.90, 0.78, 0.96)
	monitoring = true
