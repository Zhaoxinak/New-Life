extends Node
## Picks and resolves narrative events.
...
var _fired_this_period: Dictionary = {} # id -> true; prevent instant re-show loops
...
func resolve_choice(choice_id: String) -> void:
	...
	## Event choices use owner_type "choice" in effects.csv
	...
	# Defer pulse so UI can close first; same-period re-pick skips _fired_this_period.
	call_deferred("pulse")
...
func _pick_event() -> Dictionary:
	...
	## Weighted among top priority band
